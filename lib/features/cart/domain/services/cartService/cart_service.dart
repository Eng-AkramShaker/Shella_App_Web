...
class CartService implements CartServiceInterface{
  ...
  Future<List<CartItem>> getCartItems() async {
    try{
      ...
      if(response!.statusCode==200){
        if (kDebugMode) {
          print('Fetch Cart Items Response :: ${response.body}');
        }
        return (jsonDecode(response.body)['items']).map<CartItem>((json) {return CartItem.fromJson(json);}).toList();
      }else{
        throw Exception('Fetch Cart Items Failed');
      }
    }catch(e){
      if (kDebugMode) {
        print('Fetch Cart Items Exception :: ${e.toString()}');
      }
      throw Exception('Fetch Cart Items Exception :: ${e.toString()}');
    }
  }
  ...
  Future<void> addItemToCart({String? itemId, int? quantity, List<String>? variations, List<String>? addOns,
    ...
    try{
      Response? response = await cartRepository.addToCart(itemId: itemId!,quantity: quantity!,variations: variations,addOns: addOns);
      if (kDebugMode) {
  ...
  Future<void> updateCartItem(String itemId, int quantity) async {
    try{
      Response? response = await cartRepository.updateCartItem(itemId,quantity);
      if (kDebugMode) {
