...
class HomeService extends HomeServiceInterface {
  ...
  ///-------------------------------------<<<---GET POPULAR STORES--->>>-------------------------------------
  @override
  Future<PopularStoresModel> getPopularStores({String? type}) async {
    try{
      Response? response = await homeRepositoryInterface.getPopularStores(type: type);
      if(response!.statusCode==200){
  ...
  ///-------------------------------------<<<---GET STORES--->>>-------------------------------------
  @override
  Future<StoresModel> getStores({String? featured}) async {
    try{
      Response? response = await homeRepositoryInterface.getStores(featured: featured);
      if(response!.statusCode==200){
